...
public class Controller : MonoBehaviour
{
    ...
    [Header("Black Hole Slow Effect")]
    [Tooltip("The tag used by black hole objects.")]
    public string blackHoleTag = "BlackHole";

    [Tooltip("Speed multiplier when the player is near a black hole. 0.5 means half speed.")]
    [Range(0.05f, 1f)]
    public float blackHoleSlowMultiplier = 0.4f;

    private int blackHoleOverlapCount = 0;

    private bool isNearBlackHole
    {
        get
        {
            return blackHoleOverlapCount > 0;
        }
    }
    ...
    private float CurrentMoveSpeed
    {
        get
        {
            return isNearBlackHole ? moveSpeed * blackHoleSlowMultiplier : moveSpeed;
        }
    }
    ...
    private void MovePlayer(Vector3 movement)
    {
        ...
        else
        {
            ...
            Vector2 moveDelta = new Vector2(movement.x, movement.y) * Time.deltaTime * currentSpeed;

            if (myRigidbody != null)
            {
                myRigidbody.MovePosition(myRigidbody.position + moveDelta);
            }
            else
            {
                transform.position = transform.position + (movement * Time.deltaTime * currentSpeed);
            }
        }
    }
    ...
    private void OnTriggerExit2D(Collider2D other)
    {
        if (other.CompareTag(blackHoleTag))
        {
            ...
            if (blackHoleOverlapCount < 0)
            {
                blackHoleOverlapCount = 0;
            }
        }
    }
}
