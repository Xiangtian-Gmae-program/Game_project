// Script note: Runs the main-game encounter flow, enemy waves, boss phase updates, and level completion checks.
// Comment pass: documents responsibilities and key entry points without changing runtime logic.
using UnityEngine;
using UnityEngine.AI;

// Class responsibility: MainGameEncounterController contains this script's gameplay behavior.
public class MainGameEncounterController : MonoBehaviour
{
    // Data purpose: Data packet for one enemy step in the main-game encounter flow.
    ...
    // Returns the current enemy in the encounter flow.
    ...
    // Reports whether the current encounter flow is complete.
    public static bool IsComplete()
    {
        return instance != null && instance.complete;
    }

    // Reports whether the flow is waiting for the next level input.
    public static bool IsWaitingNext()
    {
        return instance != null && instance.waitingNext;
    }

    // Returns the current chapter label for UI display.
    ...
    // Returns the current target name for UI display.
    ...
    // Binds the player and first enemy template, then prepares the encounter sequence.
    ...
    // Builds the encounter steps for the current main-game level.
    ...
    // Creates one configured encounter step.
    ...
    // Spawns and configures the next enemy in the sequence.
    ...
    // Calculates a spawn position for an encounter step.
    ...
    // Applies encounter values to a spawned enemy instance.
    ...
    // Updates boss phase behavior based on remaining health.
